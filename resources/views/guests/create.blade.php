...
<div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
    <h1 class="text-2xl font-bold text-gray-800 mb-6">Add Guest</h1>
    ...
    <form action="{{ route('guests.store', $wedding->id) }}" method="POST" class="bg-white shadow-md rounded-lg p-6 space-y-6">
        @csrf
        ...
        <div>
            ...
            <input type="number" id="party_size" name="party_size" value="{{ old('party_size', 1) }}" min="1" required
        ...
        <div>
            ...
            <input type="text" id="phone" name="phone" value="{{ old('phone') }}" pattern="^\+?[0-9]{1,4}?[-. ]?(\([0-9]{1,4}\)[-. ]?)?([0-9]{1,4}[-. ]?){2,4}$"
        ...
        <div>
            ...
            <select name="rsvp_status" id="rsvp_status" required
                ...
                <option value="pending" {{ old('rsvp_status') == 'pending' ? 'selected' : '' }}>Pending</option>
                <option value="attending" {{ old('rsvp_status') == 'attending' ? 'selected' : '' }}>Attending</option>
                <option value="declined" {{ old('rsvp_status') == 'declined' ? 'selected' : '' }}>Declined</option>
            </select>
            @error('rsvp_status')
                <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
            @enderror
